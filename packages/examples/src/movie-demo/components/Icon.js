...
function url(url_) {
  return {
    background: `transparent url(${ROOT_URL}/icons/${url_}) no-repeat`,
...
export default function Icon({ type: inputType, size = 'small' }) {
  if (!ICONS.hasOwnProperty(size)) {
    throw new Error(`Invalid icon size "${size}"`)
  }
  let type = inputType.toLowerCase().replace(/[^a-z]+/g, '')
...
Icon.propTypes = { type: PropTypes.string, size: PropTypes.string }
